...
if (!class_exists('currency_updater_settings')) :
	/**
	 * currency_updater_settings class
	 */
	class currency_updater_settings
	{
		...
		{
			global $currency_updater;
			if (!is_file($settings_file)) {
				exit(__('Settings file could not be found.', 'currency-updater'));
			}
			...
			if ($option_group) {
				...
				$this->option_group = $currency_updater->get_option_group(basename($settings_file, '.php'));
			}

			$this->settings = array();
			$this->settings = apply_filters('currency_updater_register_settings', $this->settings);
			if (!is_array($this->settings)) {
				exit(__('Settings framework must be an array', 'currency-updater'));
			}
		...
		public function generate_setting($args)
		{
			...
			$this->setting_defaults = apply_filters('currency_updater_defaults', $this->setting_defaults);
			...
			do_action('currency_updater_before_field');
			do_action('currency_updater_before_field_' . $field_id);
			switch ($type) {
				case 'text':
				...
				default:
					...
					do_action('currency_updater_custom_field');
					break;
			}
			do_action('currency_updater_after_field');
			do_action('currency_updater_after_field_' . $field_id);
		}
		...
		public function settings()
		{
			...
			if (isset($_POST['currency_updater_uninstall'])) {
				check_admin_referer('currency-updater-uninstall', 'currency-updater-uninstall-nonce');
				delete_option($this->option_group);
?>
				<div class="updated">
					<p><?php _e('All options have been removed from the database.', 'currency-updater'); ?>

						<?php
						if (defined('CURRENCY_UPDATER_PLUGIN_SLUG') && CURRENCY_UPDATER_PLUGIN_SLUG != '') {
							$deactivate_url = 'plugins.php?action=deactivate&amp;plugin=' . CURRENCY_UPDATER_PLUGIN_SLUG . '/' . CURRENCY_UPDATER_PLUGIN_SLUG . '.php';
							$deactivate_url = wp_nonce_url($deactivate_url, 'deactivate-plugin_' . CURRENCY_UPDATER_PLUGIN_SLUG . '/' . CURRENCY_UPDATER_PLUGIN_SLUG . '.php');
						} else {
							$deactivate_url = admin_url('plugins.php');
						}
						?>

						<?php printf(__('To complete the uninstall <a href="%1$s"">deactivate %2$s.</a>', 'currency-updater'), esc_url($deactivate_url), CURRENCY_UPDATER_PLUGIN_NAME); ?>
					</p>
				</div>
			<?php
				return;
			...
			if (isset($_POST['currency_updater_reset'])) {
				check_admin_referer('currency-updater-reset', 'currency-updater-reset-nonce');
				delete_option($this->option_group);
			?>
				<div class="updated">
					<p><?php _e('All options have been restored to their default values.', 'currency-updater'); ?></p>
				</div>
			<?php
			}

			do_action('currency_updater_before_settings');
			?>
			<form action="options.php" method="post">
				<?php do_action('currency_updater_before_settings_fields'); ?>
				<?php settings_fields($this->option_group); ?>
				<?php do_settings_sections($this->option_group); ?>
				<?php do_action('currency_updater_after_settings_fields'); ?>

				<p class="submit">
					<input type="submit" class="button-primary" value="<?php _e('Save Changes', 'currency-updater'); ?>" />

					<?php if ($this->show_reset_button == TRUE) : ?>
						<input class="button-secondary" type="button" value="<?php _e('Restore Defaults', 'currency-updater'); ?>" onclick="document.getElementById('currency-updater-reset').style.display = 'block';document.getElementById('currency-updater-uninst').style.display = 'none';" />
					<?php endif; ?>

					<?php if ($this->show_uninstall_button == TRUE) : ?>
						<input class="button-secondary" type="button" value="<?php _e('Uninstall', 'currency-updater'); ?>" onclick="document.getElementById('currency-updater-uninst').style.display = 'block';document.getElementById('currency-updater-reset').style.display = 'none';" />
					<?php endif; ?>
				</p>
			</form>

			<div id="currency-updater-reset" style="display:none; clear: both;">
				<form method="post" action="">
					<?php wp_nonce_field('currency-updater-reset', 'currency-updater-reset-nonce'); ?>
					<label style="font-weight:normal;">
						<?php printf(__('Do you wish to <strong>completely reset</strong> the default options for', 'currency-updater')); ?> <?php echo CURRENCY_UPDATER_PLUGIN_NAME ?>? </label>
					<input class="button-secondary" type="button" name="cancel" value="<?php _e('Cancel', 'currency-updater'); ?>" onclick="document.getElementById('currency-updater-reset').style.display='none';" style="margin-left:20px" />
					<input class="button-primary" type="submit" name="currency_updater_reset" value="Restore Defaults" />
				</form>
			</div>
			<div id="currency-updater-uninst" style="display:none; clear: both;">
				<form method="post" action="">
					<?php wp_nonce_field('currency-updater-uninstall', 'currency-updater-uninstall-nonce'); ?>
					<label style="font-weight:normal;">
						<?php echo sprintf(__('Do you wish to <strong>completely uninstall</strong>', 'currency-updater')); ?> <?php echo CURRENCY_UPDATER_PLUGIN_NAME ?>?</label>
					<input class="button-secondary" type="button" name="cancel" value="<?php _e('Cancel', 'currency-updater'); ?>" onclick="document.getElementById('currency-updater-uninst').style.display = 'none';" style="margin-left:20px" />
					<input class="button-primary" type="submit" name="currency_updater_uninstall" value="Uninstall" />
				</form>
			</div>
<?php
			do_action('currency_updater_after_settings');
		}
	}
endif;
